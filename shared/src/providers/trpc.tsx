    ...
    trpc.createClient({
      links: [
        httpBatchLink({
          ...
          // attribution header — the terminal's current operator (Phase 4);
          // read per request so switching operator takes effect immediately
          headers() {
            const operator = getCurrentOperator();
            return operator ? { "x-gt-operator": operator } : {};
          },
        }),
      ],
    }),
